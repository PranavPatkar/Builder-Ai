...
export async function chat(req, res){
    ...
    try {
        //Build compact manifest (path + hash + size) instad of sending all code.
        ...
        `(${manifest.length} files, ~${JSON.stringify(manifest).length} chars)`,
    );
        ...
        project.status = "completed";
        await project.save();
        res.status(500).json({error:err.message || "Failed to process reision request"});
    }
}